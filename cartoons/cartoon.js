...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch blog.json and filter only cartoons category
    fetch('/blog.json')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            cartoonsBlogs = data.filter(blog => 
                blog.category.toLowerCase() === 'cartoons'
            );
            renderBlogs();
            setupPagination();
        })
        .catch(error => {
            blogPostsContainer.innerHTML = 
                `<p style="color:red;text-align:center;font-size:1.4em;">
                    Error loading cartoons: ${error.message}
                </p>`;
            console.error('cartoons page error:', error);
        });
    ...
    function renderBlogs() {
        ...
        const blogsToShow = cartoonsBlogs.slice(start, end);

        if (blogsToShow.length === 0) {
            blogPostsContainer.innerHTML = 
                '<p style="text-align:center;font-size:1.6em;color:#bbb;">No cartoons found yet!</p>';
            return;
        }
    ...
    function setupPagination() {
        ...
        const totalPages = Math.ceil(cartoonsBlogs.length / blogsPerPage);
