...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch blog.json and filter only trends category
    fetch('/blog.json')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            trendsBlogs = data.filter(blog => 
                blog.category.toLowerCase() === 'trends'
            );
            renderBlogs();
            setupPagination();
        })
    ...
    function renderBlogs() {
        ...
        const blogsToShow = trendsBlogs.slice(start, end);

        if (blogsToShow.length === 0) {
            blogPostsContainer.innerHTML = 
                '<p style="text-align:center;font-size:1.6em;color:#bbb;">No trends found yet!</p>';
            return;
        }
    ...
    function setupPagination() {
        ...
        const totalPages = Math.ceil(trendsBlogs.length / blogsPerPage);
